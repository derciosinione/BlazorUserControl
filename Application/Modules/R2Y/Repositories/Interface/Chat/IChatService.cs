...
public interface IChatService
{
    ...
    Task<Result<IReadOnlyList<IGetAllUserRoom_AllChatRoomByUserEmail>?>> GetAllUserRoom(
        CancellationToken cancellationToken = default);

    Task<Result<ICreatePrivateRoom_CreatePrivateRoom>> CreatePrivateRoomAsync(string userEmail, string creatorEmail,
        CancellationToken cancellationToken = default);

    IObservable<IOperationResult<IOnChatEventResult>> SubscribeToMessages(string currentUserEmail);
}
